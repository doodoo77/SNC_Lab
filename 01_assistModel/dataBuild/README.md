...
**Step 1. 텍스트 및 이미지 전처리**  
진단 보고서(PPTX)에 보이는 텍스트(검사 항목, 오류 유형, 개선방안 텍스트, 개선방안 코드)를 AI로 전사함. 오류 영역 페이지를 추출하고, URL에 접속하여 오류 영역이 포함된 전체 페이지 이미지를 함께 수집함

**Step 2. 추론 생성 (AI 기반)**  
(추론 생성을 위해 GPT 5 사용) 진단 상황마다 추론은 달라질 수 있으나, 일반적으로 웹페이지의 목적을 고려하므로 AI 모델이 전체 페이지를 참고하여 추론을 자동 생성함

**Step 3. 학습데이터 구성 및 저장**  
각 슬라이드의 사용자 입력과 추론 과정을 더한 모델 출력의 합으로 학습데이터를 구성하여 `JSONL` 형식으로 저장함
